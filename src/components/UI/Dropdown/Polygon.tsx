import React from 'react';
import styles from './polygon.module.scss';

interface Polygon {
    className?: string;
    internalClass?: string;
}

const Polygon: React.FC<Polygon> = ({ className, internalClass }) => {
    return (
        <svg className={className} xmlns="http://www.w3.org/2000/svg" viewBox="2 -3 21 21">
            <g style={{ isolation: 'isolate' }}>
                <g>
                    <image
                        width="21"
                        height="21"
                        xlinkHref='data:image/png;base64,'
                        style={{ opacity: 0.5, mixBlendMode: 'multiply' }} />
                    {/* <polygon points="3.92 7.7 13.92 2.7 13.92 12.7 3.92 7.7" style={{ fill: '#d4d4d4' }} /> */}
                    <polygon points="3.92 7.7 13.92 2.7 13.92 12.7 3.92 7.7" className={`${ styles.color } ${internalClass}`} />
                    {/* <polygon points="3.92 7.7 13.92 2.7 13.92 12.7 3.92 7.7" className={`${ styles.color }`} /> */}
                </g>
            </g>
        </svg>
    )
}

export default Polygon;