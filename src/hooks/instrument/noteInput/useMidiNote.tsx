...
export const useMidiNote = (
    ...
    ) => void, noteOffCallback: (noteNumber: number, noteName: string) => void,
) => {
    ...
    useEffect(() => {
        const v = 
            midi.channel !== 'all'  && !Number.isNaN(Number(midi.channel)) 
            ? Number(midi.channel) 
            : midi.channel
        ...
        if (v && midi.device && midi.device !== 'onboardKey') {
            ...
            if (
                ...
                ref_midiInput.current.addListener(
                    'noteon', 
                    midi.channel, 
                    midiInCallback
                );
            ...
            return () => {
                if (ref_midiInput.current
                    && midi.channel
                    && ref_midiInput.current.hasListener(
                        'noteon', 
                        midi.channel, 
                        midiInCallback
                    )
                ) {
                    ref_midiInput.current.removeListener(
                        'noteon', 
                        midi.channel, 
                        midiInCallback
                    );
